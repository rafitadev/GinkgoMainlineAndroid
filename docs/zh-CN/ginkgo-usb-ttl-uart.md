...
**杜邦线是什么：** 常见的彩色排线，两端是可插拔的塑料头（母对母、公对母）。用来插 USB-TTL 模块的排针，**不要直接焊到测试点上**——线芯比焊盘粗。
...
**漆包线是什么：** 很细的铜线，外面有一层绝缘漆。淘宝/拼多多搜 `漆包线 0.1mm`。测试点只有 **0.3–0.8 mm**，杜邦线线芯太粗、手扶也焊不稳，必须用漆包线（或 30AWG 航模电子线）焊上去。

**去漆才能上锡：** 端头的漆不刮掉，焊锡挂不住。只处理要焊的 **2–3 mm**：打火机轻烤后用砂纸或刀片刮；或烙铁蘸助焊剂把漆烫掉再镀锡。不要把整段漆都刮光，否则容易短路。

**和杜邦线怎么接：** 漆包线一端焊测试点，另一端焊到杜邦针，或绞在「母对母」杜邦线上，再插到 USB-TTL 排针。
...
2. 设备管理器看 COM 口号  
3. 用 [PuTTY](https://www.putty.org/)：连接类型选「串口 / Serial」，速率 115200
...
1. 打开 LLDM516 板级图 / 点位图（boardview）或 PCB 图，搜索 `TP0003`、`TP0012`。  
2. 对照主板丝印；附近找可靠 GND（螺丝孔金属环通常可用）。  
3. 万用表确认：未上电时 TP 对地非短路；上电后 TX≈1.8V。

> 若暂时没有板级图：可先只焊 **疑似 DBG 测试点 + GND**，上电听串口；无输出再换点。**切勿**把 3.3V TX 乱戳一通。

### 步骤 3：焊接飞线（漆包线）

1. 烙铁温度约 300–330℃，蘸助焊剂。  
2. 漆包线端头先去漆、镀锡；测试点上挂极少量锡，再把线贴上去焊住。  
3. 另一端焊到杜邦针，或绞在杜邦线上，再插 USB-TTL。  
4. 高温胶带固定，避免拉扯扯掉焊盘。  
